...
const OrganDonation: React.FC = () => {
  ...
  const fetchDonors = async () => {
    const donorList = await actor.get_all_DonorsOD();
    setDonors(donorList);
  };

  const fetchRecipients = async () => {
    const recipientList = await actor.get_all_RecipientOD();
  ...
  const handleDonorSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!actor) return;
    await actor.set_Donor_registrationOD({
        ...donorForm,
  ...
  const handleRecipientSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!actor) return;
    await actor.set_Recipient_registrationOD({
        ...recipientForm,
      ...
      <Box sx={{ my: 4 }}>
        ...
        {tabIndex === 0 && (
          <Box component="form" onSubmit={handleDonorSubmit} sx={{ mt: 3 }}>
            <Typography variant="h6">Organ Donor Registration</Typography>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}><TextField name="organ" label="Organ to Donate" fullWidth onChange={handleDonorFormChange} /></Grid>
        ...
        {tabIndex === 1 && (
          <Box component="form" onSubmit={handleRecipientSubmit} sx={{ mt: 3 }}>
            <Typography variant="h6">Organ Request</Typography>
            <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}><TextField name="bloodGroup" label="Blood Group" fullWidth onChange={handleRecipientFormChange} /></Grid>
                <Grid item xs={12} sm={6}><TextField name="reqorgan" label="Organ Required" fullWidth onChange={handleRecipientFormChange} /></Grid>
                ...
                <Grid item xs={12}><TextField name="medicalHistory" label="Medical History" fullWidth multiline rows={4} onChange={handleRecipientFormChange} /></Grid>
        ...
        {tabIndex === 2 && (
          <Box sx={{ mt: 3 }}>
            <Typography variant="h6">All Organ Donors</Typography>
            <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={4} key={index}>
                  <Card>
                    <CardContent>
                      ...
                      <Typography>Organ: {donor.organ}</Typography>
        ...
        {tabIndex === 3 && (
          <Box sx={{ mt: 3 }}>
            <Typography variant="h6">All Organ Requests</Typography>
            <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={4} key={index}>
                  <Card>
                    <CardContent>
                      ...
                      <Typography>Required Organ: {recipient.reqorgan}</Typography>
                      <Typography>Blood Group: {recipient.bloodGroup}</Typography>
